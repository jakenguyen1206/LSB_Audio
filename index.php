	...
	<?php
		...
		class WavFile{
			...
			public static function ReadFile($filename) {
	            ...
	            $wav = array(
	                    'header'    => array(
	                        'chunkid'       => self::readString($handle, 4),	#Get "RIFF"
	                        'chunksize'     => self::readLong($handle),			#ReadLong = 4 bytes
	                        'format'        => self::readString($handle, 4)		#Get "WAVE"
	                        ),
	                    'subchunk1' => array(		#subchunk1 = FORMAT CHUNK
	                        'id'            => self::readString($handle, 4),	#Get "fmt"
	                        'size'          => self::readLong($handle),			#ReadLong 4 bytes Subchunk1 size
	                        'audioformat'   => self::readWord($handle),			#ReadWord 2 bytes Audio Format
	                        'numchannels'   => self::readWord($handle),			#ReadWord 2 bytes Channel
	                        'samplerate'    => self::readLong($handle),			#ReadLong 4 bytes SampleRate
	                        'byterate'      => self::readLong($handle),			#ReadLong 4 bytes Byte/s
	                        'blockalign'    => self::readWord($handle),			#ReadWord 2 bytes Byte per Sample 
	                        'bitspersample' => self::readWord($handle)			#ReadWord 2 bytes Bit Deep (8bit - 16bit)
	                        ),
	                    'subchunk2' => array( 		#subchunk2 = DATA CHUNK
	                        'id'            => self::readString($handle, 4),	#Get "data"
	                        'size'			=> self::readLong($handle),			#GetLong 4 bytes DataSize
	                        'data'          => null
	                        ),
	                    'subchunk3' => array(
	                    	'id'			=> null,
	...
	<body>
		<section class="navbar">
		    <div class="container">
				...
				<?php
					if (isset($_SESSION['user'])){
						echo "<a id=\"goto-login\" href=\"logout.php\" title=\"Sign out\"><img src=\"picture/goto-logout.png\"/></a>";
		    ...
		    <div class="container">
		      	<ul class="nav mainbar-list wow fadeInDown" data-wow-duration="0.5s">
		        	<li id="buy-more-song" class="active">
		          		<a href="#buymoresong" title="Buy more songs">
		            		<i class="fa fa-headphones"></i> Buy songs
		          		</a>
		       		</li>
		       		<li id="get-signature">
		          		<a href="#getsignature" title="Get signature">
		            		<i class="fa fa-asl-interpreting"></i> Get sign
		          		</a>
		       		</li>
		       		<?php
		...
		<div class="mycontent">
			...
			<form class="getsignature" action="" method="post" enctype="multipart/form-data" style="display: none;">
                ...
                <label for="upfile-input-file" class="btn btn-primary"><i class="fa fa-search"></i> From Computer</label>
			...
			<form class="uploadnewsong" action="" method="post" enctype="multipart/form-data" style="display: none;">
                ...
                <label for="upfile-input-file-1" class="btn btn-success"><i class="fa fa-search"></i> From Computer</label>
                <label id="upfile-file-name-1" style="display: none;"></label>
                <button class="btn btn-success" name="upfilebtn1"><i class="fa fa-upload"></i> Upload</button>
			</form>
		</div>
